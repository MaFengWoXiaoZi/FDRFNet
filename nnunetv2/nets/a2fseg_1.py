...
from nnunetv2.nets.a2fseg.multi_modal.utils import subset_idx

from copy import deepcopy
from nnunetv2.utilities.v1.nd_softmax import softmax_helper
from torch import nn
import torch
import numpy as np
from nnunetv2.nets.a2fseg.initialization import InitWeights_He
from nnunetv2.nets.a2fseg.my.neural_network3 import SegmentationNetwork
...
##############################################################################
    ...
    def forward(self, x, subset_idx_list=[14], num = 4):
        ...
        num = self.modality_num
##############################################################################
        drop = np.array([True if k in SUBSETS_MODALITIES[subset_idx_list[0]] else False for k in range(num)]) == False
##############################################################################
        # print(drop)

        x = torch.chunk(x, num, dim = 1)
