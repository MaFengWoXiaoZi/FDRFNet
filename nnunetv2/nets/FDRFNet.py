...
def concatenate_multimodal_features(
    ...
    """
    Concatenate features from different modalities and different hierarchy levels along the channel dimension.

    Assumptions:
        Each modality extracts features at each level, and all modalities share the same spatial shape
        (D, H, W) at the same level.

    Args:
        features_by_modality (List[List[torch.Tensor]]):
            A list containing features from all modalities.
            The outer list index corresponds to modality.
            The inner list index corresponds to level (from shallow to deep).
            Example: [[mod1_feat_L1, mod1_feat_L2], [mod2_feat_L1, mod2_feat_L2]]

    Returns:
        List[torch.Tensor]:
            A list containing fused (concatenated) features at each level.
            For each level, the channel dimension equals the sum of channels across modalities.
    """
    if not features_by_modality:
        return []

    num_modalities = len(features_by_modality)
    num_levels = len(features_by_modality[0])  # assume all modalities have the same number of levels

    fused_features_per_level: List[torch.Tensor] = []

    # Iterate over each level (e.g., from the shallowest to the deepest)
    for level_idx in range(num_levels):
        # Collect features of all modalities at the current level
        features_at_current_level = []
        for mod_idx in range(num_modalities):
            # Ensure the current modality has a feature tensor at this level
            if level_idx < len(features_by_modality[mod_idx]):
                features_at_current_level.append(features_by_modality[mod_idx][level_idx])
            else:
                raise ValueError(
                    f"Modality {mod_idx} is missing features at level {level_idx}. "
                    "Please ensure all modalities provide corresponding features at all levels."
                )

        # Concatenate modality features at this level along channel dimension (dim=1)
        # Example: if each modality outputs C channels, concatenating M modalities yields M*C channels.
...
class HighFrequencyFeatureExtractor(nn.Module):
    def __init__(self, cutoff_ratio: float = 0.05):
        """
        Initialize the high-frequency feature extractor.

        Args:
            cutoff_ratio (float): Cutoff frequency ratio for the Gaussian high-pass filter.
                                  Smaller values preserve more low-frequency information and less high-frequency.
    ...
    def _create_3d_gaussian_high_pass_filter(self, D: int, H: int, W: int, device: torch.device) -> torch.Tensor:
        """
        Create a 3D Gaussian high-pass filter.

        Args:
            D (int): Depth dimension size.
            H (int): Height dimension size.
            W (int): Width dimension size.
            device (torch.device): Tensor device (CPU or CUDA).

        Returns:
            torch.Tensor: A 3D Gaussian high-pass filter of shape (D, H, W).
        """
        # Create frequency grids, typically in [-0.5, 0.5)
        ...
        # Use torch.meshgrid to create a 3D frequency coordinate grid
        # indexing='ij' ensures the dimension order matches (D, H, W)
        d_grid, h_grid, w_grid = torch.meshgrid(d_coords, h_coords, w_coords, indexing='ij')

        # Compute squared normalized distance to the frequency center (0, 0, 0)
        distance_sq = d_grid**2 + h_grid**2 + w_grid**2

        # Gaussian high-pass: G(d) = 1 - exp(-d^2 / (2*sigma^2))
        # sigma controls transition smoothness; usually related to cutoff
        sigma = self.cutoff_ratio / 3.0  # heuristic; can be tuned experimentally

        gaussian_low_pass = torch.exp(-distance_sq / (2 * sigma**2))
        high_pass_filter = 1 - gaussian_low_pass

        # Ensure filter values are within [0, 1]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass: extract high-frequency features and concatenate them with the original input.

        Args:
            x (torch.Tensor): Input image tensor of shape (b, c, h, w, d).
                              dtype should be torch.float32.

        Returns:
            torch.Tensor: Concatenated features (original + high-frequency) of shape (b, 2*c, h, w, d).
        """
        if x.dim() != 5:
            raise ValueError(f"Input tensor must be 5D (b, c, h, w, d), but got {x.dim()}D.")

        # Extract shape information
        b, c, h, w, d = x.shape
        device = x.device  # tensor device

        # 1) Apply 3D FFT along (h, w, d)
        # dim=(-3, -2, -1) indicates FFT over h, w, d
        # Output is complex (complex64/complex128 depending on input)
        ...
        # 3) Create and apply a 3D Gaussian high-pass filter
        # The filter is created over (h, w, d) and broadcast over batch and channels.
        high_pass_filter = self._create_3d_gaussian_high_pass_filter(h, w, d, device)

        # Multiply by filter; broadcasting applies automatically
        filtered_freq_data_shifted = freq_data_shifted * high_pass_filter

        # 4) Shift back for inverse FFT
        filtered_freq_data = torch.fft.ifftshift(filtered_freq_data_shifted, dim=(-3, -2, -1))

        # 5) Inverse 3D FFT back to spatial domain
        high_freq_spatial = torch.fft.ifftn(filtered_freq_data, dim=(-3, -2, -1))

        # Use the real part as high-frequency features, cast to float32
        high_freq_features = high_freq_spatial.real.to(torch.float32)

        # 6) Concatenate original and high-frequency features along channel dimension
        # Original: (b, c, h, w, d), High-freq: (b, c, h, w, d) => Output: (b, 2c, h, w, d)
...
class PlainConvUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            # Channel-level orthogonality loss:
            # compute orthogonality between the two feature types at each layer using the Frobenius norm
            # of the cross-covariance matrix as the orthogonality penalty.
            ...
            # Spatial-level orthogonality loss for each layer
...
if __name__ == '__main__':
    ...
    # print(f'Original tensor shape: {dummy_input.shape}')
