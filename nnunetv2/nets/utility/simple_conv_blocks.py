...
class GroupedDropout(nn.Module):
    def __init__(self, p = 0.1, groups = 1, inplace = False):
        super(GroupedDropout, self).__init__()
        self.p = p
        self.groups = groups
    ...
    def forward(self, x):
        ...
        for i in range(self.groups):
            x_group = x[:, i * group_channels: (i + 1) * group_channels]
            x_group = F.dropout3d(x_group, p = self.p, training = self.training, inplace = self.inplace)
            out.append(x_group)

        return torch.cat(out, dim = 1)

class GroupConvDropoutNormReLU(nn.Module):
    def __init__(self,
        ...
        super(GroupConvDropoutNormReLU, self).__init__()
        ...
        if dropout_op is not None:
            ...
            p = dropout_op_kwargs['p']
            inplace = dropout_op_kwargs['inplace']
            self.dropout = GroupedDropout(p = p, groups = groups, inplace = inplace)
            ops.append(self.dropout)

        if norm_op is not None:
            eps = norm_op_kwargs['eps']
            affine = norm_op_kwargs['affine']
            self.norm = nn.GroupNorm(num_groups = groups, num_channels = output_channels, eps = eps, affine = affine)
    ...
    def __init__(self,
        ...
        if groups is not None:
            self.convs = nn.Sequential(
                GroupConvDropoutNormReLU(
                    conv_op, input_channels, output_channels[0], kernel_size, initial_stride, conv_bias, norm_op,
                    norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, nonlin_first, groups = groups
                ),
                *[
                    GroupConvDropoutNormReLU(
