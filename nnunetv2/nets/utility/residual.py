...
from .simple_conv_blocks import ConvDropoutNormReLU, GroupConvDropoutNormReLU
...
class BasicBlockD(nn.Module):
    def __init__(self,
        ...
        if groups is None:
            ...
            self.conv1 = GroupConvDropoutNormReLU(conv_op, input_channels, output_channels, kernel_size, stride, conv_bias,
                                            norm_op, norm_op_kwargs, dropout_op, dropout_op_kwargs, nonlin, nonlin_kwargs, groups = groups)
            self.conv2 = GroupConvDropoutNormReLU(conv_op, output_channels, output_channels, kernel_size, 1, conv_bias, norm_op,
                                            norm_op_kwargs, None, None, None, None, groups = groups)
        ...
        if block == BasicBlockD:
            blocks = nn.Sequential(
                block(conv_op, input_channels, output_channels[0], kernel_size, initial_stride, conv_bias,
                    ...
                    squeeze_excitation, squeeze_excitation_reduction_ratio, groups = groups),
                *[block(conv_op, output_channels[n - 1], output_channels[n], kernel_size, 1, conv_bias, norm_op,
                        ...
                        squeeze_excitation, squeeze_excitation_reduction_ratio, groups = groups) for n in range(1, n_blocks)]
            )
        else:
            blocks = nn.Sequential(
