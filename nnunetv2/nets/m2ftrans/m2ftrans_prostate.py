...
from .layers_prostate import (general_conv3d, normalization, prm_generator, prm_fusion,
                    prm_generator_laststage, region_aware_modal_fusion, fusion_postnorm)
from .blocks_prostate import nchwd2nlc2nchwd, DepthWiseConvBlock, ResBlock, GroupConvBlock, MultiMaskAttentionLayer, MultiMaskCrossBlock
from torch.nn.init import constant_, xavier_uniform_
from .mask_prostate import mask_gen_fusion
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.e5_c1 = general_conv3d(basic_dims*8, basic_dims*16, stride=(2, 2, 1), pad_type='reflect')
...
class Decoder_fusion(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.RFM3 = fusion_postnorm(in_channel=basic_dims*4, num_cls=num_cls - 1)
        self.RFM2 = fusion_postnorm(in_channel=basic_dims*2, num_cls=num_cls - 1)
        self.RFM1 = fusion_postnorm(in_channel=basic_dims*1, num_cls=num_cls - 1)
...
class MaskedAttention(nn.Module):
    def __init__(
        self, dim, heads=8, qkv_bias=False, qk_scale=None, dropout_rate=0.0, num_class=2
    ):
        super().__init__()
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, mask, fusion, pos):
        ...
        embed_adc = adc.flatten(2).transpose(1, 2).contiguous()

        embed_cat = torch.cat((embed_t2, embed_adc, fusion), dim=1)
        embed_cat = embed_cat + pos
        embed_cat_trans, attn = self.trans_bottle(embed_cat, mask)
        t2_trans, adc_trans, fusion_trans = torch.chunk(embed_cat_trans, self.num_cls+1, dim=1)

        return t2_trans, adc_trans, fusion_trans, attn
...
class Weight_Attention(nn.Module):
    ...
    def forward(self, de_x1, de_x2, de_x3, de_x4, de_x5, attn):
        ...
        attn_0 = attn[0]
        attn_fusion = attn_0[:, :, ((H//patch_size[0]) * (W//patch_size[1]) * (D//patch_size[2]))*2 :, :]
        attn_t2, attn_adc, attn_self = torch.chunk(attn_fusion, num_modals+1, dim=-1)

        attn_t2 = torch.sum(torch.sum(attn_t2, dim=1), dim=-2).reshape(adc_tra.size(0), (H//patch_size[0]), (W//patch_size[1]), (D//patch_size[2])).unsqueeze(dim=1)
        attn_adc = torch.sum(torch.sum(attn_adc, dim=1), dim=-2).reshape(t2_tra.size(0), (H//patch_size[0]), (W//patch_size[1]), (D//patch_size[2])).unsqueeze(dim=1)

        dex5 = (t2_tra*(attn_t2), adc_tra*(attn_adc))

        # attn_t2, attn_adc = self.upsample(attn_t2), self.upsample(attn_adc)
        attn_t2, attn_adc = F.interpolate(attn_t2, scale_factor=(2, 2, 1), mode='nearest'), F.upsample(attn_adc, scale_factor=(2, 2, 1), mode='nearest')
        dex4 = (t2_x4*(attn_t2), adc_x4*(attn_adc))

        attn_t2, attn_adc = self.upsample(attn_t2), self.upsample(attn_adc)
        dex3 = (t2_x3*(attn_t2), adc_x3*(attn_adc))

        attn_t2, attn_adc = self.upsample(attn_t2), self.upsample(attn_adc)
        dex2 = (t2_x2*(attn_t2), adc_x2*(attn_adc))

        attn_t2, attn_adc = self.upsample(attn_t2), self.upsample(attn_adc)
        dex1 = (t2_x1*(attn_t2), adc_x1*(attn_adc))
...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.pos = nn.Parameter(torch.zeros(1, ((H//patch_size[0]) * (W//patch_size[1]) * (D//patch_size[2]))*3, basic_dims*16))
        self.fusion = nn.Parameter(nn.init.normal_(torch.zeros(1, ((H//patch_size[0]) * (W//patch_size[1]) * (D//patch_size[2])), basic_dims*16), mean=0.0, std=1.0))
    ...
    def forward(self, x, mask):
        ...
        x_bottle = (t2_x5, adc_x5)

        B = x.size(0)
        fusion = torch.tile(self.fusion, [B, 1, 1])

        t2_trans, adc_trans, fusion_trans, attn = self.Bottleneck(x_bottle, mask, fusion, self.pos)

        t2_tra = t2_trans.view(x.size(0), (H//patch_size[0]), (W//patch_size[1]), (D//patch_size[2]), basic_dims*16).permute(0, 4, 1, 2, 3).contiguous()
        adc_tra = adc_trans.view(x.size(0), (H//patch_size[0]), (W//patch_size[1]), (D//patch_size[2]), basic_dims*16).permute(0, 4, 1, 2, 3).contiguous()
       
        fusion_tra = fusion_trans.view(x.size(0), (H//patch_size[0]), (W//patch_size[1]), (D//patch_size[2]), basic_dims*16).permute(0, 4, 1, 2, 3).contiguous()


        de_x5 = (t2_tra, adc_tra)
        de_x4 = (t2_x4, adc_x4)
        de_x3 = (t2_x3, adc_x3)
        de_x2 = (t2_x2, adc_x2)
        de_x1 = (t2_x1, adc_x1)
        ...
        if self.is_training:
            ...
            prm_preds = [pred.permute(0, 1, 4, 2, 3).contiguous() for pred in prm_preds]

            return fuse_pred, (t2_pred, adc_pred), prm_preds
        
        return fuse_pred
    
if __name__ == '__main__':
    model = Model(num_cls = 3).cuda()
    y = model(torch.randn(1, 2, 256, 256, 16).cuda(), torch.tensor([[True, False]]).cuda())
    print(y.shape)