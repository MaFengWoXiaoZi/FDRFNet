...
from .mask_prostate import mask_gen_cross4
...
class MultiMaskCrossBlock(nn.Module):
    ...
    def forward(self, kernels, feature_maps, mask):
        ...
        if self.ffn_feature_maps:
            ...
            adc = self.ffn2(adc, identity=adc)
            feature_maps = (t2, adc)
...
class MultiMaskAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask):
        ...
        qb, qc, qh, qw, qd = query.shape
        query = self.query_map(query).flatten(2)
        key_t2 = self.key_map_flair(t2).flatten(2)
        value_t2 = self.value_map_flair(t2).flatten(2)
        key_adc = self.key_map_t1ce(adc).flatten(2)
        value_adc = self.value_map_t1ce(adc).flatten(2)

        key = torch.cat((key_t2, key_adc), dim=1)
        value = torch.cat((value_t2, value_adc), dim=1)
