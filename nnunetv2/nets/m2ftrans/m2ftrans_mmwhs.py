...
from .blocks_mmwhs import nchwd2nlc2nchwd, DepthWiseConvBlock, ResBlock, GroupConvBlock, MultiMaskAttentionLayer, MultiMaskCrossBlock
...
class Decoder_fusion(nn.Module):
    def __init__(self, num_cls=2):
        ...
        self.RFM3 = fusion_postnorm(in_channel=basic_dims*4, num_cls=2)
        self.RFM2 = fusion_postnorm(in_channel=basic_dims*2, num_cls=2)
        self.RFM1 = fusion_postnorm(in_channel=basic_dims*1, num_cls=2)
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x, mask, fusion, pos):
        ct, mri = x
        embed_ct = ct.flatten(2).transpose(1, 2).contiguous()
        embed_mri = mri.flatten(2).transpose(1, 2).contiguous()

        embed_cat = torch.cat((embed_ct, embed_mri, fusion), dim=1)
        embed_cat = embed_cat + pos
        embed_cat_trans, attn = self.trans_bottle(embed_cat, mask)
        ct_trans, mri_trans, fusion_trans = torch.chunk(embed_cat_trans, self.num_cls+1, dim=1)

        return ct_trans, mri_trans, fusion_trans, attn
...
class Weight_Attention(nn.Module):
    ...
    def forward(self, de_x1, de_x2, de_x3, de_x4, de_x5, attn):
        ...
        attn_ct, attn_mri, attn_self = torch.chunk(attn_fusion, num_modals+1, dim=-1)

        attn_ct = torch.sum(torch.sum(attn_ct, dim=1), dim=-2).reshape(ct_tra.size(0), (H//16), (W//16), (D//16)).unsqueeze(dim=1)
        attn_mri = torch.sum(torch.sum(attn_mri, dim=1), dim=-2).reshape(ct_tra.size(0), (H//16), (W//16), (D//16)).unsqueeze(dim=1)


        dex5 = (ct_tra*(attn_ct), mri_tra*(attn_mri))

        attn_ct, attn_mri = self.upsample(attn_ct), self.upsample(attn_mri)
        dex4 = (ct_x4*(attn_ct), mri_x4*(attn_mri))

        attn_ct, attn_mri = self.upsample(attn_ct), self.upsample(attn_mri)
        dex3 = (ct_x3*(attn_ct), mri_x3*(attn_mri))

        attn_ct, attn_mri = self.upsample(attn_ct), self.upsample(attn_mri)
        dex2 = (ct_x2*(attn_ct), mri_x2*(attn_mri))

        attn_ct, attn_mri = self.upsample(attn_ct), self.upsample(attn_mri)
        dex1 = (ct_x1*(attn_ct), mri_x1*(attn_mri))
...
class Model(nn.Module):
    def __init__(self, num_cls=2):
        ...
        self.pos = nn.Parameter(torch.zeros(1, ((H//16) * (W//16) * (D//16))*3, basic_dims*16))
    ...
    def forward(self, x, mask):
        ...
        x_bottle = (ct_x5, mri_x5)

        B = x.size(0)
        fusion = torch.tile(self.fusion, [B, 1, 1])

        ct_trans, mri_trans, fusion_trans, attn = self.Bottleneck(x_bottle, mask, fusion, self.pos)

        ct_tra = ct_trans.view(x.size(0), (H//16), (W//16), (D//16), basic_dims*16).permute(0, 4, 1, 2, 3).contiguous()
        mri_tra = mri_trans.view(x.size(0), (H//16), (W//16), (D//16), basic_dims*16).permute(0, 4, 1, 2, 3).contiguous()
        fusion_tra = fusion_trans.view(x.size(0), (H//16), (W//16), (D//16), basic_dims*16).permute(0, 4, 1, 2, 3).contiguous()


        de_x5 = (ct_tra, mri_tra)
        de_x4 = (ct_x4, mri_x4)
        de_x3 = (ct_x3, mri_x3)
        de_x2 = (ct_x2, mri_x2)
        de_x1 = (ct_x1, mri_x1)
        ...
        if self.is_training:
            ...
            return fuse_pred, (ct_pred, mri_pred), prm_preds
        return fuse_pred
    
if __name__ == '__main__':
    x = torch.randn(1, 4, 128, 128, 128).cuda()
    model = Model(num_cls = 4).cuda()
    y = model(x, torch.tensor([[True, False, True, True]]).cuda())
    print(y.shape)