...
class MultiMaskCrossBlock(nn.Module):
    ...
    def forward(self, kernels, feature_maps, mask):
        ...
        if self.ffn_feature_maps:
            ct = self.ffn2(ct, identity=ct)
            mri = self.ffn2(mri, identity=mri)
            feature_maps = (ct, mri)
...
class MultiMaskAttentionLayer(nn.Module):
    def __init__(self,
        ...
        self.attn_drop = nn.Dropout(attn_drop)
        self.query_map = DepthWiseConvBlock(query_dim, query_dim)
        self.key_map_ct = DepthWiseConvBlock(kv_dim, kv_dim)
        self.value_map_ct = DepthWiseConvBlock(kv_dim, kv_dim)
        self.key_map_mri = DepthWiseConvBlock(kv_dim, kv_dim)
        self.value_map_mri = DepthWiseConvBlock(kv_dim, kv_dim)
    ...
    def forward(self, query, key, value, mask):
        ...
        qb, qc, qh, qw, qd = query.shape
        query = self.query_map(query).flatten(2)
        key_ct = self.key_map_ct(ct).flatten(2)
        value_ct = self.value_map_ct(ct).flatten(2)
        key_mri = self.key_map_mri(mri).flatten(2)
        value_mri = self.value_map_mri(mri).flatten(2)

        key = torch.cat((key_ct, key_mri), dim=1)
        value = torch.cat((value_ct, value_mri), dim=1)
