...
class nnUNetPredictor(object):
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.trainer_name == 'nnUNetTrainerMMFormer' or self.trainer_name == 'nnUNetTrainerM2FTrans' or self.trainer_name == 'nnUNetTrainerRFNet':
            if self.dataset_json['name'] == 'BRATS':
                mask = torch.tensor([[True, True, True, True]])
            elif self.dataset_json['name'] == 'PROSTATE' or self.dataset_json['name'] == 'MMWHS':
                mask = torch.tensor([[True, True]])
            prediction = self.network(x, mask)
        elif self.trainer_name == 'nnUNetTrainerUMambaEncMe':
            _, prediction = self.network(x)
        elif self.trainer_name == 'nnUNetTrainerOurs':
        ...
        if mirror_axes is not None:
            ...
            if isinstance(self.network, HDenseFormer) or isinstance(self.network, Generic_MAML_multi3_channel):
                prediction = prediction[0]
                for axes in axes_combinations:
                    prediction += torch.flip(self.network(torch.flip(x, (*axes,)))[0], (*axes,))
            elif self.trainer_name == 'nnUNetTrainerMMFormer' or self.trainer_name == 'nnUNetTrainerM2FTrans' or self.trainer_name == 'nnUNetTrainerRFNet':
                for axes in axes_combinations:
                    prediction += torch.flip(self.network(torch.flip(x, (*axes,)), mask), (*axes,))
            elif self.trainer_name == 'nnUNetTrainerUMambaEncMe':
                for axes in axes_combinations:
                    prediction += torch.flip(self.network(torch.flip(x, (*axes,)))[1], (*axes,))
            elif self.trainer_name == 'nnUNetTrainerOurs':
                for axes in axes_combinations:
                    prediction += torch.flip(self.network(torch.flip(x, (*axes,)))[0], (*axes,))
