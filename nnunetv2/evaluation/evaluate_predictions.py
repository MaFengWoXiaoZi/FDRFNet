...
def compute_hd95(pred_mask, true_mask):
    ...
    dist_pred_to_true = cdist(pred_points, true_points, metric = 'euclidean')
    min_dist_pred_to_true = np.min(dist_pred_to_true, axis = 1)

    dist_true_to_pred = cdist(true_points, pred_points, metric = 'euclidean')
    min_dist_true_to_pred = np.min(dist_true_to_pred, axis = 1)

    all_distances = np.concatenate([min_dist_pred_to_true, min_dist_true_to_pred])
...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
    ...
    for r in labels_or_regions:
        ...
        # results['metrics'][r]['HD95'] = compute_hd95(mask_pred, mask_ref)
    return results
