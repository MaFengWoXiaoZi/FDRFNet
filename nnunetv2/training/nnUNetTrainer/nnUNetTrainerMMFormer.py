...
from nnunetv2.nets.mmformer import mmformer_brats, mmformer_prostate, mmformer_mmwhs
...
class nnUNetTrainerMMFormer(nnUNetTrainer):
    def __init__(
        self,
        plans: dict,
        configuration: str,
    ...
    def build_network_architecture(plans_manager,
        ...
        if dataset_json['name'] == 'BRATS':
            model = mmformer_brats.Model(num_cls = len(dataset_json['labels']))
        elif dataset_json['name'] == 'PROSTATE':
            model = mmformer_prostate.Model(num_cls = len(dataset_json['labels']))
        elif dataset_json['name'] == 'MMWHS':
            model = mmformer_mmwhs.Model(num_cls = len(dataset_json['labels']))
        return model
