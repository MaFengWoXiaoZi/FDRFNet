...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, L1_norm, L2_norm
from nnunetv2.training.loss.deep_supervision import DeepSupervisionWrapper
from nnunetv2.training.loss.dice import get_tp_fp_fn_tn, MemoryEfficientSoftDiceLoss
from nnunetv2.training.lr_scheduler.polylr import PolyLRScheduler, CosineAnnealingLRScheduler, CosineAnnealingWithWarmupLRScheduler
...
import nnunetv2.nets.criterions as criterions
...
class nnUNetTrainer(object):
    ...
    def _get_deep_supervision_scales(self):
        if self.enable_deep_supervision:
            # from nnunetv2.training.nnUNetTrainer.nnUNetTrainerA2Fseg import nnUNetTrainerA2Fseg
            # if isinstance(self, nnUNetTrainerA2Fseg) and self.dataset_json['name'] == 'PROSTATE':
            #     self.configuration_manager.pool_op_kernel_sizes = [[1, 1, 1], [2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2], [2, 2, 2]]
    ...
    def _build_loss(self):
        ...
        if self.enable_deep_supervision:
            ...
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerA2Fseg import nnUNetTrainerA2Fseg
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerHDenseFormer import nnUNetTrainerHDenseFormer

            if isinstance(self, nnUNetTrainerA2Fseg):
                weights = np.asarray(weights.tolist()[:1] + weights.tolist() * (self.num_input_channels + 1))
            # elif isinstance(self, nnUNetTrainerHDenseFormer):
    ...
    def configure_optimizers(self):
        ...
        # optimizer = torch.optim.AdamW(self.network.parameters(), lr = self.initial_lr, weight_decay=self.weight_decay, betas = (0.9, 0.999))
        # lr_scheduler = PolyLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        # lr_scheduler = CosineAnnealingLRScheduler(optimizer, self.initial_lr, self.num_epochs)
        lr_scheduler = CosineAnnealingWithWarmupLRScheduler(optimizer, self.initial_lr, self.num_epochs)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerA2Fseg import nnUNetTrainerA2Fseg
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerMMFormer import nnUNetTrainerMMFormer
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerHDenseFormer import nnUNetTrainerHDenseFormer
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerM2FTrans import nnUNetTrainerM2FTrans
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerUMambaEncMe import nnUNetTrainerUMambaEncMe
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerOurs import nnUNetTrainerOurs
            if isinstance(self, nnUNetTrainerA2Fseg):
                output = self.network(data, num = self.num_input_channels)
                tmp_target = [target[0], ]
                for i in range(self.num_input_channels + 1):
                    tmp_target += target
                target = tmp_target
            elif isinstance(self, nnUNetTrainerMMFormer) or isinstance(self, nnUNetTrainerM2FTrans) or isinstance(self, nnUNetTrainerRFNet):
                b, m = data.shape[0:2]
                self.network.is_training = True
                # self.network.module.is_training = True
                mask = ~torch.zeros(b, m).type(torch.bool)
                fused_pred, sep_preds, prm_preds = self.network(data, mask)
            elif isinstance(self, nnUNetTrainerHDenseFormer):
                output = self.network(data)
                target = target[:-1]
            elif isinstance(self, nnUNetTrainerOurs):
                # output, pred_imgs, (shared_features, dis_features) = self.network(data)
                # output = self.network(data)
                output, pred_imgs, ortho_loss = self.network(data)
            else:
                output = self.network(data)
            # del data
            
            if isinstance(self, nnUNetTrainerMMFormer) or isinstance(self, nnUNetTrainerM2FTrans):
                target = F.one_hot(target.squeeze(1).long(), num_classes = len(self.dataset_json['labels'])).permute(0, 4, 1, 2, 3).contiguous()
                l = self.loss(fused_pred, sep_preds, prm_preds, target, len(self.dataset_json['labels']))
            elif isinstance(self, nnUNetTrainerUMambaEncMe):
                # l = nn.functional.mse_loss(output[0], data) + (1 - ssim(output[0], data, data_range = 1, size_average = True))
                l = nn.functional.mse_loss(output[0], data)
                l = l + self.loss(output[1], target)
                # l = l + L1_norm(self.network.module.shared_decoder) + L1_norm(self.network.module.multi_modal_encoder)
            elif isinstance(self, nnUNetTrainerOurs):
                rec_loss = nn.functional.l1_loss(pred_imgs, data)

                # sim_loss = torch.Tensor([0.]).cuda()
                # for i in range(len(shared_features)):
                #     sim_loss += torch.abs(torch.cosine_similarity(shared_features[i], dis_features[i]).mean())
                    # sim_loss += torch.clamp(torch.cosine_similarity(shared_features[i], dis_features[i]), min = 0.).mean()

                # seg_loss =  self.loss(output, target)
                # l = rec_loss + sim_loss + seg_loss
                l = self.loss(output, target) + 0.5 * ortho_loss + 0.5 * rec_loss
            else:
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerMMFormer import nnUNetTrainerMMFormer
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerM2FTrans import nnUNetTrainerM2FTrans
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerRFNet import nnUNetTrainerRFNet
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerUMambaEncMe import nnUNetTrainerUMambaEncMe
            from nnunetv2.training.nnUNetTrainer.nnUNetTrainerOurs import nnUNetTrainerOurs
            if isinstance(self, nnUNetTrainerA2Fseg):
                output = self.network(data, num = self.num_input_channels)
                ...
                # self.network.module.is_training = False
                ...
                output = self.network(data)
            # del data
            if isinstance(self, nnUNetTrainerMMFormer) or isinstance(self, nnUNetTrainerM2FTrans) or isinstance(self, nnUNetTrainerRFNet):
                l = torch.tensor([0.0])
            elif isinstance(self, nnUNetTrainerUMambaEncMe):
                l = nn.functional.mse_loss(output[0], data)
                l = l + self.loss(output[1], target)
                ...
                # sim_loss = torch.Tensor([0.]).cuda()
                # for i in range(len(shared_features)):
                #     sim_loss += torch.cosine_similarity(shared_features[i], dis_features[i]).mean()

                # seg_loss =  self.loss(output, target)

                # l = rec_loss + sim_loss + seg_loss
                # l = self.loss(output, target)
                l = self.loss(output, target) + 0 * ortho_loss + 0 * rec_loss
            else:
                l = self.loss(output, target)

        # we only need the output with the highest output resolution (if DS enabled)
        if self.enable_deep_supervision:
            if isinstance(self, nnUNetTrainerUMambaEncMe):
                output = output[1][0]
            else:
                output = output[0]
            target = target[0]
